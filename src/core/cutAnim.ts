...
/** A cut's entrance, exit, deform and move, as the cut panel edits them. */
export interface CutAnimSettings {
    inType: string; inDur: number; inDir: string;
    outType: string; outDur: number; outDir: string;
    deformAxis: string; deformAmount: number; deformReturn: boolean; deformSpeed: number; deformCount: number;
    moveX: number; moveY: number; moveReturn: boolean; moveSpeed: number; moveCount: number;
    ease: string; easePower: number;
}
/** The cut's transform at a moment: nothing to apply is null, not the identity. */
export interface CutAnimAt { alpha: number; sx: number; sy: number; tx: number; ty: number }


export const ANIM_DEFAULT: CutAnimSettings = { inType: 'none', inDur: 0.4, inDir: 'left', outType: 'none', outDur: 0.4, outDir: 'right', deformAxis: 'x', deformAmount: 0, deformReturn: false, deformSpeed: 1, deformCount: 0, moveX: 0, moveY: 0, moveReturn: false, moveSpeed: 1, moveCount: 0, ease: 'linear', easePower: 2 };
...
export function computeCutAnim(ac: { anim?: CutAnimSettings | null } & TimeSpan, time: number, cw = CANVAS_W, ch = CANVAS_H): CutAnimAt | null {
    const a = ac.anim;
    if (!a) return null;
    ...
    const dirOff = (dir: string | undefined, frac: number): [number, number] => dir === 'left' ? [-cw * frac, 0] : dir === 'right' ? [cw * frac, 0] : dir === 'up' ? [0, -ch * frac] : [0, ch * frac];
