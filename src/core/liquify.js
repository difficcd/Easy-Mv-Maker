 ...
 */
export function sampleBilinear(src, w, h, x, y, out) {
    const x0 = Math.floor(x), y0 = Math.floor(y);
    const fx = x - x0, fy = y - y0;
    const at = (px, py, c) => (px < 0 || py < 0 || px >= w || py >= h) ? 0 : src[(py * w + px) * 4 + c];
    for (let c = 0; c < 4; c++) {
 ...
 */
export function pushPixels(buf, w, h, { x, y, r, dx, dy, strength = 1 }) {
 ...
 */
export function pushAlong(buf, w, h, from, to, r, strength = 1) {
