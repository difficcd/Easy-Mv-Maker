...
export class ByteWriter {
    ...
    u8(v: number): void { this._room(1); this.buf[this.len++] = v & 255; }
    u16(v: number): void { this._room(2); this.buf[this.len++] = v & 255; this.buf[this.len++] = (v >> 8) & 255; }
    u32(v: number): void {
        this._room(4);
    ...
    str(text: string): void { this._room(text.length); for (let i = 0; i < text.length; i++) this.buf[this.len++] = text.charCodeAt(i); }
    bytes(arr: Uint8Array): void { this._room(arr.length); this.buf.set(arr, this.len); this.len += arr.length; }
    /** How many bytes have been written, which is also the offset the next one lands at. */
    get position(): number { return this.len; }
    /** Backed by a plain ArrayBuffer, ready for a Blob. */
