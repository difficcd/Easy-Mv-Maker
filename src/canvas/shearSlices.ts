...
/** One slice of a sheared span: where it starts, how long it is, and the shear `k * pos + m` across it. */
export interface ShearSlice { a0: number; len: number; k: number; m: number }
 ...
 */
export function shearSlices(dispAt: (pos: number) => number, start: number, span: number, slices: number): ShearSlice[] {
    const out: ShearSlice[] = [];
