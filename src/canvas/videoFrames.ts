...
/** A video file opened for seeking: the element, how long it is, a seek that resolves when the frame is there, and the release. */
export interface OpenVideo { video: HTMLVideoElement; duration: number; seek: (t: number) => Promise<void>; release: () => void }
/** What an extraction produces: encoded frames, how many steps each is held for, and the size they were decoded at. */
export interface ExtractedFrames { frames: Blob[]; holds: number[]; skipped: number; fps: number; duration: number; width: number; height: number }


// Letterbox rect: fit source into destination preserving aspect ratio.
export function fitRect(sw: number, sh: number, dw: number, dh: number): Rect {
 ...
 */
export async function openVideoFile(file: Blob): Promise<OpenVideo> {
...
export async function extractVideoFrames(file: Blob, { fps = 6, maxFrames = 0, start = 0, end = null, width, height, scale = 1, quality = 0.82, dedupe = 'exact', nativeRes = false, format = 'webp', onProgress, shouldStop }: Partial<ExtractOptions> & { onProgress?: (done: number, total: number, skipped: number) => void, shouldStop?: () => boolean } = {}): Promise<ExtractedFrames> {
    ...
    try {
        ...
        const vW = video.videoWidth || Math.round((width ?? 0) * scale), vH = video.videoHeight || Math.round((height ?? 0) * scale);
        const useNative = nativeRes && video.videoWidth && video.videoHeight;
        const fw = useNative ? vW : Math.max(1, Math.round((width ?? 0) * scale));
        const fh = useNative ? vH : Math.max(1, Math.round((height ?? 0) * scale));
        ...
        const toBlob = format === 'png'
            ? () => new Promise<Blob | null>((res) => cnv.toBlob(res, 'image/png'))                    // lossless
            : () => new Promise<Blob | null>((res) => cnv.toBlob(b => b ? res(b) : cnv.toBlob(res, 'image/jpeg', quality), 'image/webp', quality));
        ...
        const frames: Blob[] = [], holds: number[] = [];
        let prevSig: Uint8Array | null = null, prevFull: Uint8ClampedArray | null = null, skipped = 0;
 ...
 */
export async function detectSceneCuts(file: Blob, { start = 0, end = null, step = 0.2, threshold = 14, refine = true, onProgress, shouldStop }: { start?: number, end?: number | null, step?: number, threshold?: number, refine?: boolean, onProgress?: (done: number, total: number) => void, shouldStop?: () => boolean } = {}): Promise<number[]> {
    ...
    try {
        ...
        const diff = (a: Uint8Array, b: Uint8Array) => { let s = 0; for (let i = 0; i < a.length; i++) s += Math.abs(a[i] - b[i]); return s / a.length; };
        // Binary-search the exact moment the picture stops matching the pre-cut frame → precise boundary.
        const refineCut = async (refSig: Uint8Array, a: number, b: number) => {
        ...
        let prev: Uint8Array | null = null, prevT = from;
