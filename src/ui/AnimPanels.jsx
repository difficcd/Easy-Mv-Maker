    ...
    return (
        <div style={{ padding: '6px 8px', background: 'hsl(var(--ui-h) var(--ui-s) 10%)', borderTop: '1px solid hsl(var(--ui-h) var(--ui-s) 20%)', display: 'flex', flexDirection: 'column', gap: 5 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={R('#8bd')} title={tr('지지직 — 이 레이어의 선이 찢어지고 색이 갈라집니다. 선이 있는 곳에만 걸립니다')}>
                ...
                <NumIn label={tr('색')} value={round2(a.noiseColor || 0)} onChange={v => updLayerAnim(cut.id, layer.id, { noiseColor: Math.max(0, Math.min(1, v)) })} step={0.1} min={0} w={46} title={tr('0~1. 선 위에 파랑·초록 점이 튑니다')} />
            </div>
            {!!a.noise && (
                <div style={R('#8bd')}>
                    <span style={{ width: 24, flexShrink: 0 }} />
