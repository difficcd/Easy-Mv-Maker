...
export default function App() {
    ...
    const buildData = () => {
        ...
        usedIds.forEach(id => {
            const entry = bitmapStoreRef.current.get(id);
            if (entry?.imageData) bitmaps[id] = imageDataToDataURL(entry.imageData);
        });
        return {
    ...
    // Option lists so animation values are chosen, not typed.
                                ...
                                <div key={cut.id} className={`cut-item${currentCutId === cut.id ? ' cut-active' : ''}`} onClick={() => setCurrentCutId(cut.id)}>
                                    ...
                                    {expandedCuts.has(cut.id) && (
                                        <div className="cut-settings" onClick={e => e.stopPropagation()}>
                                            ...
                                            {(() => { const a = { ...ANIM_DEFAULT, ...cut.anim }; return (
                                    ...
                                    >
                                        ...
                                        {cuts.filter(c => (c.track || 0) === ti).map(cut => (
                                            <div key={cut.id}
                                                ...
                                                onClick={e => { e.stopPropagation(); setCurrentCutId(cut.id); }}
                                                onPointerDown={e => { e.stopPropagation(); setCurrentCutId(cut.id); cutDragMovedRef.current = false; clearTimeout(cutDragTimerRef.current); cutDragArmedRef.current = e.pointerType !== 'touch'; if (e.pointerType === 'touch') cutDragTimerRef.current = setTimeout(() => { cutDragArmedRef.current = true; }, 350); e.currentTarget.setPointerCapture(e.pointerId); setDraggingCutData({ cutId: cut.id, startX: e.clientX, startY: e.clientY, initialStart: cut.startTime, initialTrack: cut.track }); }}>
