...
/** What the playback loop reads, grouped by where it comes from. */
export interface PlaybackDeps {
    /** the elements and what is loaded into them */
    media: { audioRef: { current: HTMLAudioElement | null }, videoElRef: { current: HTMLVideoElement | null }, audioUrl: string | null, audioData: AudioClip | null, videoOverlay: VideoOverlay | null };
    /** where playback runs, how fast, and where pressing play rewinds to */
    range: { playStart: number, playEnd: number, maxTime: number, loopPlay: boolean, playbackRate: number, anchorTime: number | undefined };
    /** where to draw each frame */
    paint: { pps: number, playheadRef: { current: HTMLElement | null }, paintFrameRef: { current: ((t: number, playing: boolean) => void) | null }, prefetchRef: { current: ((t: number, playing: boolean) => void) | null } };
    /** export state; playback runs at real time while recording, whatever speed is selected */
    recording: { isExporting: { current: boolean }, exportEndRef: { current: number }, exportStartRef: { current: number }, requestFrameRef: { current: (() => void) | null }, mediaRecorderRef: { current: MediaRecorder | null } };
}
 ...
 *   export state; playback runs at real time while recording, whatever speed is selected
 */
export function usePlayback({ media, range, paint, recording }: PlaybackDeps) {
    ...
    const seekRef = useRef<number | null>(null);  // a scrub while playing lands here
    ...
    useEffect(() => {
        ...
        const syncVideo = (tt: number, forceSeek: boolean) => {
        ...
        syncVideo(t, true);
        const finish = (end: number) => { isPlayingRef.current = false; setIsPlaying(false); if (audio) audio.pause(); if (vid) vid.pause(); setCurrentTime(end); currentTimeRef.current = end; paintFrameRef.current?.(end, false); };
