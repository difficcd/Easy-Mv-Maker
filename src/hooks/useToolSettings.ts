 ...
 */
export function useToolSettings({ busy, leaveCurve }: { busy: () => boolean, leaveCurve: () => void }) {
    ...
    const setRulerMode = (m: string) => { leaveCurve(); setRulerModeRaw(m); };
    ...
    const [recentColors, setRecentColors] = useStored<string[]>('mv_recent_colors', [], arrayCodec);
    const [pickingColor, setPickingColor] = useState(false); // eyedropper: next canvas click samples a pixel
    const applyColor = (c: string | null | undefined) => { if (!c) return; setColor(c); };
    // "Used" means something was actually drawn in that colour; only then does it join Recent.
    const noteColorUsed = (c: string | null | undefined) => {
        if (!c) return;
    ...
    const setToolSize = (n: number) => { const v = clampBrush(n); if (tool === 'eraser') setEraserSize(v); else setBrushSize(v); };

    const handleSetTool = (newTool: string) => {
        if (busy()) return;
