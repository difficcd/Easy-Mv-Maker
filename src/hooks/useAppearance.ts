 ...
 */
export function useAppearance({ applyTheme, defaultTheme }: { applyTheme: (colour: string, uiSat: number) => void, defaultTheme: string }) {
    const [themeColor, setThemeColor] = useStored('mv_theme', defaultTheme);
    const [themeRecent, setThemeRecent] = useStored<string[]>('mv_theme_recent', [], arrayCodec);
