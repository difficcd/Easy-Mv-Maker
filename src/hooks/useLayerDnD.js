 ...
 */
export function useLayerDnD({ updLayers }) {
    const [dragLayerInfo, setDragLayerInfo] = useState(null);   // {cutId, layerId} in flight
    const [dropInfo, setDropInfo] = useState(null);             // {layerId, position} under the pointer
    const clear = () => { setDragLayerInfo(null); setDropInfo(null); };

    const onLayerDragStart = (e, cutId, layerId) => {
        e.stopPropagation();
