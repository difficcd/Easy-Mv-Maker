...
/** What is under the pointer on a floating selection, or what a press turned into. */
export type SelectionHit = { type: 'rotate' } | { type: 'resize', handle: string } | { type: 'move' } | { type: 'warp' };
 ...
 */
export function useSelectionGesture({ gesture, selection, setSelection, zoom }: { gesture: Gesture, selection: any, setSelection: (f: (s: any) => any) => void, zoom: number }) {
    /**
     ...
     */
    const hitTest = (pos: Point): SelectionHit | null => {
        ...
        const near = (p: Point) => Math.abs(pos.x - p.x) <= grab && Math.abs(pos.y - p.y) <= grab;
     ...
     */
    const begin = (e: PressEvent, pos: Point): boolean => {
        ...
        const kind: SelectionHit = warp ? { type: 'warp' } : hit;
