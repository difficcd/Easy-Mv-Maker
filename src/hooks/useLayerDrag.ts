...
/** What a press hands the gesture hooks: the pointer to capture, and the default to suppress. */
export interface PressEvent { pointerId: number; preventDefault: () => void; ctrlKey?: boolean; metaKey?: boolean }
 ...
 */
export function useLayerDrag({ gesture, cuts, dispatchCuts, setDragTick, liveCtx, clearLiveOverlay, ensureLayerCanvas }: { gesture: Gesture, cuts: Cut[], dispatchCuts: (action: CutsAction) => void, setDragTick: (f: (v: number) => number) => void, liveCtx: () => CanvasRenderingContext2D | null, clearLiveOverlay: () => void, ensureLayerCanvas: (cutId: Id, layer: Layer) => HTMLCanvasElement | null }) {
    /**
