...
/** A touch gesture on the canvas area: one finger panning, or two pinching. */
type TouchGesture =
    | { mode: 'pan', startPt: Point, startView: View }
    | { mode: 'pinch', startDist: number, startMid: Point, startView: View };
/** What the area's pointer handlers read: React's PointerEvent has all of it, and so does the DOM's. */
export interface AreaPointer { pointerType: string; pointerId: number; button: number; clientX: number; clientY: number; preventDefault: () => void }
 ...
 */
export function useCanvasView({ canvasAreaRef }: { canvasAreaRef: { current: HTMLElement | null } }) {
    const [view, setView] = useState<View>({ zoom: 1, x: 0, y: 0 });
    const [spaceDown, setSpaceDown] = useState(false);   // space = pan (hand) mode
    const spaceDownRef = useRef(false);
    const panningRef = useRef(false);
    const lastInteractRef = useRef(0);
    const touchPtsRef = useRef(new Map<number, Point>());
    ...
    const zoomCanvas = (factor: number) => { touched(); setView(v => zoomAbout(v, factor)); };
    ...
    const startMousePan = (e: { clientX: number, clientY: number }) => {
        const sx = e.clientX, sy = e.clientY, sv = { ...view };
        panningRef.current = true;
        const mv = (ev: PointerEvent) => { touched(); setView({ zoom: sv.zoom, x: sv.x + (ev.clientX - sx), y: sv.y + (ev.clientY - sy) }); ev.preventDefault(); };
    ...
    const onAreaPointerDown = (e: AreaPointer) => {
    ...
    const onAreaPointerMove = (e: AreaPointer) => {
    ...
    const onAreaPointerUp = (e: AreaPointer) => {
