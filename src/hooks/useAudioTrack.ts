...
export function useAudioTrack({ audioUrl, dispatchMedia, setLinkPrompt }: { audioUrl: string | null, dispatchMedia: (action: MediaAction) => void, setLinkPrompt: (p: { kind: 'audio' }) => void }) {
    ...
    const audioBlobRef = useRef<{ src: string | null, blob: Blob | null }>({ src: null, blob: null });
    ...
    const audioAsBlob = async (): Promise<Blob | null> => {
     ...
     */
    const loadAudioUrl = (url: string, name: string, startAt = 0, offset = 0, clipDur: number | null = null): void => {
        ...
        else { fetch(url).then(r => r.blob()).then(b => { const fr = new FileReader(); fr.onload = () => { audioB64Ref.current = fr.result as string; }; fr.readAsDataURL(b); }).catch(() => { }); }
    };
     ...
     */
    const restoreAudio = async (data: any, assetBase: string | null = null): Promise<number> => {
    ...
    const handleAudioUpload = (e: { target: { files: FileList | null } }) => {
    ...
    const loadYoutubeAudio = async (presetUrl?: unknown): Promise<void> => {
