...
/** A long job's progress; total 0 means the length is not known yet. */
export interface Progress { label: string; done: number; total: number; [k: string]: any }
...
export function useNotices() {
    ...
    const [linkPrompt, setLinkPrompt] = useState<{ kind: 'video' | 'audio' } | null>(null);
