 ...
 */
export function useHistory<T>({ snapshot, shouldSkip, apply }: { snapshot: T, shouldSkip: () => boolean, apply: (snapshot: T) => void }): { undo: () => void, redo: () => void, record: (snapshot: T) => void, entries: () => T[] } {
    ...
    const record = useCallback((snap: T) => {
