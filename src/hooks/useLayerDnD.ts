 ...
 */
export function useLayerDnD({ updLayers }: { updLayers: (cutId: DocId | null, fn: (cut: Cut) => Partial<Cut>) => void }) {
    const [dragLayerInfo, setDragLayerInfo] = useState<{ cutId: DocId, layerId: DocId } | null>(null);   // in flight
    const [dropInfo, setDropInfo] = useState<{ layerId: DocId, position: DropPosition } | null>(null);   // under the pointer
    const clear = () => { setDragLayerInfo(null); setDropInfo(null); };

    const onLayerDragStart = (e: React.DragEvent, cutId: DocId, layerId: DocId) => {
        e.stopPropagation();
        setDragLayerInfo({ cutId, layerId });
        e.dataTransfer.effectAllowed = 'move';
    };
    const onLayerDragOver = (e: React.DragEvent<HTMLElement>, targetId: DocId, targetType: string | undefined) => {
    ...
    const onLayerDrop = (e: React.DragEvent, cutId: DocId, targetId: DocId) => {
        e.preventDefault(); e.stopPropagation();
    ...
    const onListDrop = (e: React.DragEvent, cutId: DocId) => {
        e.preventDefault();
