...
/** Where a panel lives: docked to an edge, or floating over the canvas. */
export type Dock = 'left' | 'right' | 'float';
/** A panel being dragged by its header: where the pointer is, which dock it is over, and where it was grabbed. */
export interface PanelDrag { id: string; x: number; y: number; zone: Dock; dx: number; dy: number }
/** A splitter drag in flight: a docked panel's edge, or the timeline's top. */
type Splitter = { type: 'panel', id: string, side: 'left' | 'right', startX: number, startW: number } | { type: 'bottom', startY: number, startH: number };


/** The element each panel's markup is rooted in, for finding which one a header belongs to. */
const PANEL_ROOTS: Record<string, string> = { color: '.color-panel', tools: '.toolbar', cut: '.right-panel' };
 ...
 */
export function usePanelLayout({ panelIds, widthRange }: { panelIds: readonly string[], widthRange: Record<string, number[]> }) {
    ...
    const [splitter, setSplitter] = useState<Splitter | null>(null);

    const [docks, setDocks] = useStored<Record<string, Dock>>('mv_docks', { tools: 'left', color: 'left', cut: 'right' }, {
        ...
        decode: (raw: string) => { const v = JSON.parse(raw); return v && ['left', 'right', 'float'].includes(v.tools) ? v : undefined; },
        encode: JSON.stringify,
    });
    const [floatPos, setFloatPos] = useStored<Record<string, Point>>('mv_floats',
        { tools: { x: 120, y: 120 }, color: { x: 160, y: 160 }, cut: { x: 200, y: 200 } }, {
        decode: (raw: string) => { const v = JSON.parse(raw); return v && typeof v === 'object' ? v : undefined; },
        encode: JSON.stringify,
    });
    ...
    const panelWidth: Record<string, number> = { color: colorW, tools: toolW, cut: rightW };

    /** Which dock a pointer position means; anywhere in the middle means "pull it out". */
    const dropZoneAt = (x: number): Dock => (x < DOCK_BAND ? 'left' : x > window.innerWidth - DOCK_BAND ? 'right' : 'float');
    ...
    const onDockPointerDown = (e: { button?: number, target: EventTarget | null, clientX: number, clientY: number, preventDefault: () => void }) => {
        if (e.button !== undefined && e.button !== 0) return;
        const target = e.target as HTMLElement | null;
        const head = target?.closest?.('.panel-head');
        if (!head) return;
        if (target!.closest('button, input, select, textarea')) return;   // ✕ and controls still work
        ...
        const mv = (ev: PointerEvent) => setPanelDrag(d => d && ({ ...d, x: ev.clientX, y: ev.clientY, zone: dropZoneAt(ev.clientX) }));
        dragOnWindow(mv, (ev: PointerEvent) => {
    ...
    const startPanelResize = (id: string, side: 'left' | 'right', clientX: number) =>
    ...
    const startBottomResize = (clientY: number) =>
