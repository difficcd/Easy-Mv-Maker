...
import type { Keymap, ShortcutState } from '../core/shortcuts.ts';

/** One function per action name shortcutFor can return, plus `state()` returning what there is to act on. */
export interface ShortcutActions { state: () => ShortcutState; [action: string]: ((arg?: string) => void) | (() => ShortcutState) }
 ...
 */
export function useShortcuts(keymap: Keymap, actions: ShortcutActions): void {
