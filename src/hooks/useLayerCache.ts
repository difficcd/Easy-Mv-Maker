...
/** What the layer cache reads from the app: the store, the document, where the playhead is, and what is on screen. */
export interface LayerCacheDeps {
    store: BitmapStore;
    cuts: Cut[];
    ...
    onionPrev: boolean;
    onionNext: boolean;
    activePartId: Id | null | undefined;
    isPlaying: boolean;
    isPlayingRef: { current: boolean };
    canvasW: number;
    canvasH: number;
    /** layers a gesture draws itself */
    hiddenByGesture: (cutId: Id, layerId: Id) => boolean;
    /** where the playback loop finds prefetchFramesAt */
    prefetchRef: { current: ((time: number, playing: boolean) => void) | null };
    /** the boiling phase the paint loop sets */
...
export function useLayerCache({
    ...
    const fallbackCanvasRef = useRef(new Map<string, HTMLCanvasElement>()); // LRU of layer canvases built on demand during render
    const [frameDecodeTick, setFrameDecodeTick] = useState(0); // bumped when a frame finishes decoding, so the cache rebuilds
    const clipScratchRef = useRef<HTMLCanvasElement | null>(null);
    const clipPaintRef = useRef<HTMLCanvasElement | null>(null);
    ...
    const invalidateCutsUsing = (ids: Iterable<string>) => {
    ...
    const requestFrameDecode = (ids: string[]) => {
    ...
    useEffect(() => {
        ...
        const store = bitmaps.map;
        const keep = new Set<string>(); // frame ids belonging to the active part
        cuts.forEach(c => { if (c.partId === activePartId) safeArray<Layer>(c.layers).forEach(l => safeArray<Stroke>(l.strokes).forEach(s => { if (s.tool === 'paste' && s.bitmapId) keep.add(s.bitmapId); })); });
        let released = false;
    ...
    const prefetchFramesAt = (time: number, playing: boolean) => {
    ...
    const ensureLayerCanvas = (cutId: Id, layer: Layer): HTMLCanvasElement | null => {
    ...
    const flattenClipGroup = (cutId: Id, group: { base: Layer, clipped: Layer[] }): HTMLCanvasElement | null => {
        ...
        const dragging = (id: Id) => hiddenByGesture(cutId, id);
