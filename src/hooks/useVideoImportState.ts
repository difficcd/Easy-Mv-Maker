...
/** What an import run takes from the app at call time: the document, the store, and where the result goes. */
export interface ImportRunDeps {
    cw: number;
    ch: number;
    cuts: Cut[];
    currentCutId: Id | null | undefined;
    /** bumped when another document is opened, so a run that finishes late can tell */
    docEpochRef: { current: number };
...
export function useVideoImportState() {
    ...
    const [busy, setBusy] = useState<{ done: number, total: number, skipped?: number, fetching?: boolean } | null>(null);
    ...
    const [sceneCfg, setSceneCfg] = useState<any>(null);
     ...
     */
    const restore = async (field: any, { assetBase, dispatchMedia, requestRepaint }: { assetBase: string | null, dispatchMedia: (action: MediaAction) => void, requestRepaint: () => void }): Promise<number> => {
     ...
     */
    const run = async ({ cw, ch, cuts, currentCutId, docEpochRef, setCanvasSize, storeBitmapBlob, dispatchCuts, setCurrentCutId, setCurrentTime, loadAudioUrl, gcBitmaps, notices }: ImportRunDeps): Promise<void> => {
        if (!cfg?.file) return;
