 ...
 */
export function usePanelVisibility({ timelineRef }: { timelineRef: { current: HTMLElement | null } }) {
    const [showLeft, setShowLeft] = useState(true);
    const [showRight, setShowRight] = useState(true);
    const [showBottom, setShowBottom] = useState(true);
    // Which panel is open in the left dock (null = closed); switched from the icon rail.
    const [leftDock, setLeftDock] = useState<string | null>('color');

    const beforeHideRef = useRef<{ left: boolean, dock: string | null, right: boolean, bottom: boolean } | null>(null);
    const scrollRef = useRef<{ left: number, top: number } | null>(null);
    ...
    const toggleAllPanelsRef = useRef<(() => void) | null>(null);
