...
/** What the timeline's gestures read and drive: the element, the document, the clock, the audio, and the selection. */
export interface TimelineGestureDeps {
...
export interface TimelinePress { button?: number; clientX: number; clientY: number; shiftKey?: boolean; ctrlKey?: boolean; metaKey?: boolean; preventDefault: () => void; stopPropagation: () => void }
/** A touch gesture on the timeline: one finger panning (or tapping), or two pinching. */
type TouchGesture =
    | { mode: 'pan', startClientX: number, startClientY: number, startScroll: number, moved: boolean }
    | { mode: 'pinch', startDist: number, startPps: number, anchorTime: number };
...
export function useTimelineGestures({
    ...
    const sceneTimelineTimes = (): number[] => (Array.isArray(videoOverlay?.cuts) ? videoOverlay.cuts : [])
    ...
    const startTimelinePan = (e: TimelinePress) => {
    ...
    const startTimelineScrub = (e: TimelinePress) => {
    ...
    const startMarqueeOrSeek = (e: TimelinePress) => {
        if (e.button !== undefined && e.button !== 0) return;
        const el = timelineRef.current;
        if (!el) return;
        const rect = el.getBoundingClientRect();
    ...
    const latest = useRef<{ pps: number, seekToClientX: (x: number) => void, zoomTimelineAt: (x: number, f: number) => void }>({ pps, seekToClientX: () => { }, zoomTimelineAt: () => { } });
    ...
    const zoomTimelineAt = (clientX: number, factor: number) => {
    ...
    const onTimelinePointerDown = (e: TimelinePress & { pointerType: string, target: EventTarget | null }) => {
        if (e.pointerType === 'touch') return;
        if ((e.target as HTMLElement | null)?.closest?.('.ruler')) startTimelineScrub(e);
    ...
    useEffect(() => {
        const el = timelineRef.current;
        if (!el) return;
        const pts = new Map<number, { x: number, y: number, onBlock: boolean }>();
        let gesture: TouchGesture | null = null;
        const fingers = () => [...pts.values()];
        const down = (e: PointerEvent) => {
            if (e.pointerType !== 'touch') return;
            const onBlock = !!(e.target as HTMLElement | null)?.closest?.('.cut-block, .rh, button');
