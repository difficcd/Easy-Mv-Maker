 ...
 */
export function useTimelineView({ timelineRef, mounted, height, maxTime, numTracks }: { timelineRef: { current: HTMLElement | null }, mounted: boolean, height: number, maxTime: number, numTracks: number }) {
    ...
    const [snapLinePos, setSnapLinePos] = useState<number | null>(null);
