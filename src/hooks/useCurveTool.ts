...
import type { BitmapLookup } from '../canvas/strokes.ts';
 ...
 */
export function useCurveTool({ overlay, brush, bitmapStoreRef, zoom, commitLiveStroke }: { overlay: { clear: () => void, ctx: () => CanvasRenderingContext2D | null }, brush: () => { color: string, opacity: number, size: number }, bitmapStoreRef: { current: BitmapLookup }, zoom: () => number, commitLiveStroke: (stroke: Stroke) => void }) {
    const anchorsRef = useRef<PressurePoint[] | null>(null);
    const draggingRef = useRef(false);
    const [count, setCount] = useState(0);   // anchor count, for the done/cancel bar

    const strokeFrom = (pts: PressurePoint[]): Stroke => {
    ...
    const addAnchor = (pos: PressurePoint) => {
    ...
    const dragTo = (pos: PressurePoint): boolean => {
