...
| `drawStatic` | Broken-signal static over the finished frame: bands torn sideways with the colour channels pulled apart at the tear (red one side, cyan the other — `lighter` of a red-only and a cyan-only copy of the band), plus coarse snow. Intermittent by design — most frames are clean and then one is not. Deterministic in time, so the export matches the preview. Drawn *outside* the camera transform. |
