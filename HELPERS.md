...
| `grainTile` | A square of monochrome noise centred on mid grey, built once and reused for the snow. |
| `colourTile` | A square of sparse blue-green specks on transparency, for the colour static, painted onto the ink with `source-atop`; a full-coverage tile would read as a tint, not static. |
| `staticCanvas` | A copy of a layer with broken-signal static on it, alpha preserved: the ink split into red-only and cyan-only halves and put back as `lighter` with a sideways offset, band tears on bad frames, snow on the strokes only (`source-atop`). Each half is masked back to the layer's alpha with `destination-in`, or the transparent area would come out solid red — and that is also what lets it run on a transparent background. Deterministic in time. |
